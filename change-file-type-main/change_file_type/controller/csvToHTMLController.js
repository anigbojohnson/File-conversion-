...
upload(req,res,(err)=>{
    ...
    if(err){
        ...
        async function main() {
            let outputPath = path.join('..','change_file_type','output',req.file.originalname.split('.')[0]+"_"+generatedFileName+".html")
       
            const pythonCsvToPdfConvert = new PythonShell(path.join("pythonProject","csvToHTML.py"),{
                mode: "text",
                pythonPath:'python',
